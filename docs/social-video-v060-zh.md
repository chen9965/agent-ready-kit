...
`agent-ready-kit` 是一个帮仓库适配 AI 编码代理的工具：它会扫描命令、入口、目录边界和验证方式，然后生成 Agent 能直接使用的 `AGENTS.md`、报告、任务卡、前后对比和行动计划。

更明确的解释：

它不是 linter，不是 README 生成器，也不是聊天套壳。它的重点是把仓库里的隐含规则变成可用产物，帮助 Codex、Claude Code、Cursor 这类大模型编码代理知道：该看哪里、跑什么命令、哪些文件别碰、改完怎么验证。

## 口播脚本

开场：

`agent-ready-kit` 是帮仓库适配 AI 编码代理的工具。它会扫描仓库规则，并生成 Agent 可直接使用的工作材料。
...
它会整理安装命令、测试命令、CI、项目入口、目录边界和忽略规则，还能生成 `AGENTS.md`、报告和行动计划。
...
它不是 linter，也不是 README 生成器。它是在给 AI 编码代理准备可执行的仓库说明书。
...
我做了一个开源工具 `agent-ready-kit`：扫描仓库里的命令、入口、边界和验证方式，生成 `AGENTS.md`、报告、任务卡、前后对比、行动计划和 guard rules。用了之后，AI 编码代理更少猜命令、更少改错文件、更容易按规则验证。默认免 API Key，也可以直接扫描 GitHub 仓库 URL。
...
答：npm 能访问时直接运行 `npx @chent6767/agent-ready-kit .`。默认会使用托管大模型建议；如果托管建议不可用，可以自带 OpenAI 兼容 key，最后才回退到本地扫描。
