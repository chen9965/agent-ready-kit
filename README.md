...
After scanning, it gives you a score and can generate the missing agent-facing files. For large repositories, `scan --out` also writes a portable artifact bundle: raw JSON, Markdown report, prioritized action plan, and a before/after explanation.

扫描后，它会给仓库一个就绪度分数，并且可以生成 Agent 直接能看的说明文件和任务清单。针对大仓库，`scan --out` 还会落地一组可分享产物：原始 JSON、Markdown 报告、优先级行动计划和使用前后对比说明。
