...
**Give LLM coding agents the repository context they need to edit code correctly.**

**把仓库规则整理给大模型，让 Codex、Claude Code、Cursor 这类编码代理更少猜、更少改错。**

**Default path: no model signup and no API key required.**

**默认使用路径不需要注册模型平台，也不需要用户自己申请 API Key。**

`agent-ready-kit` is a local CLI and GitHub Action that prepares repository context for LLM coding agents. It turns scattered repo knowledge into agent-readable instructions: setup commands, test commands, CI, project structure, edit boundaries, validation steps, and `AGENTS.md`.

`agent-ready-kit` 是一个本地 CLI 和 GitHub Action，用来帮大模型理解仓库。它会把分散在仓库里的信息整理成 Agent 可读的上下文：怎么安装、怎么测试、CI 在哪里、项目结构是什么、哪些边界不要乱碰、改完怎么验证、有没有 `AGENTS.md`。
...
The default path requires no model signup and no API key. Advanced users can still bring their own OpenAI-compatible key or run local-only.

默认使用路径不需要注册模型平台，也不需要用户申请 API Key；高级用户仍然可以自带 OpenAI 兼容 key，或选择纯本地扫描。
...
| LLM recommendations | Default no-key suggestions, with BYOK and local-only options for advanced users. | 默认免 key 生成建议，也支持高级用户自带 key 或纯本地模式。 |

## Quick Start / 快速开始

The default command first tries the maintainer-hosted managed LLM endpoint, so common users can run it without creating any model account or API key.

默认命令会优先请求维护者托管的大模型端点，所以普通用户可以直接运行，不需要创建模型账号或申请 API Key。
...
默认情况下，`agent-ready-kit` 可以使用托管大模型路径生成更好的建议：
...
It sends only bounded context and skips obvious secret-like paths and content. Use `--no-llm` for local-only scanning.

它只使用有限上下文，并跳过明显像密钥的路径和内容。需要纯本地扫描时，用 `--no-llm`。
...
- LLM agents start with repo-specific commands and working rules.
