...
Optional LLM mode can enrich the report when you bring your own OpenAI-compatible key:

如果你有 OpenAI 兼容接口的免费额度或自建模型，也可以开启可选大模型增强：
...
The default scan still works offline. LLM mode sends only the score, detected signals, and findings summary, not repository source files.

默认扫描仍然可以离线运行。大模型模式只发送评分、检测信号和发现摘要，不上传仓库源码文件。
...
| AI wrapper | Calls a model | Runs locally by default; LLM enrichment is optional and provider-agnostic. | 默认本地运行；大模型增强是可选的，并且不绑定服务商。 |
