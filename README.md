...
After scanning, it gives you a score and can generate the missing agent-facing files. For large repositories, `--out` also writes a portable artifact bundle: raw JSON, Markdown report, prioritized action plan, and a before/after explanation. When `AGENT_READY_LLM_API_KEY` is configured, the scanner automatically asks an OpenAI-compatible model to read bounded sampled code context, so recommendations are based on the actual project shape, not only file names.

扫描后，它会给仓库一个就绪度分数，并且可以生成 Agent 直接能看的说明文件和任务清单。针对大仓库，`--out` 还会落地一组可分享产物：原始 JSON、Markdown 报告、优先级行动计划和使用前后对比说明。只要配置了 `AGENT_READY_LLM_API_KEY`，扫描器会默认把有限采样后的代码上下文发给 OpenAI 兼容模型，让建议基于真实项目结构，而不只是文件名。
...
| Default LLM recommendations | If an API key is configured, model-written suggestions based on sampled code context. | 配置 API key 后默认启用大模型建议，并基于采样代码上下文。 |
...
Enable the default free-model route:
...
Without an API key, the scanner is deterministic and local. It reads repository shape, docs, scripts, and lightweight file evidence. With an API key, it adds bounded sampled-code LLM recommendations by default. It still does not claim to review every line like a full human reviewer.

如果没有 API key，扫描器是确定性的本地分析：只读取仓库结构、文档、脚本和轻量文件证据。如果配置了 API key，它会默认增加“有限采样代码”的大模型建议，但仍然不会假装完整审查了每一行代码。

## Default LLM Mode / 默认大模型模式

No API key is required for the local fallback. If `AGENT_READY_LLM_API_KEY` is present, LLM mode is enabled by default and sends bounded sampled code context: file tree, entrypoint candidates, test candidates, and short excerpts from selected config/source files. It skips obvious secret-like paths and content.

本地兜底不需要 API key。只要存在 `AGENT_READY_LLM_API_KEY`，大模型模式就会默认启用，并发送有限的采样代码上下文：文件树、入口候选、测试候选，以及少量配置/源码片段。它会跳过明显像密钥的路径和内容。

The shortest setup uses OpenRouter's free-model router:

最短配置是使用 OpenRouter 的免费模型路由：
...
- `AGENT_READY_LLM_PROVIDER=openrouter`
...
- **Local fallback:** no model account required if you just want deterministic scanning.
- **LLM-aware by default:** when a key exists, code-only repositories get sampled-code analysis automatically.
...
- **快：** 一条命令得到分数和修复方向。
- **本地兜底：** 只做确定性扫描时不需要模型账号。
- **默认接入大模型：** 只要有 key，纯代码仓库会自动得到采样代码分析。
