...
**LLM-first repository context for coding agents.**

**优先用大模型理解仓库，再把规则整理给 Codex、Claude Code、Cursor 这类编码代理。**

**Default path: no model signup and no API key required.**

**默认使用路径不需要注册模型平台，也不需要用户自己申请 API Key。**

`agent-ready-kit` is an LLM-first CLI and GitHub Action that prepares repository context for coding agents. The local scanner collects bounded evidence, then the LLM turns scattered repo knowledge into agent-readable instructions: setup commands, test commands, CI, project structure, edit boundaries, validation steps, and `AGENTS.md`.

`agent-ready-kit` 是一个 LLM-first 的 CLI 和 GitHub Action，用来帮大模型理解仓库。它会先在本地采集有限证据，再让大模型把分散信息整理成 Agent 可读上下文：怎么安装、怎么测试、CI 在哪里、项目结构是什么、哪些边界不要乱碰、改完怎么验证、有没有 `AGENTS.md`。

In plain language, it helps LLM agents with three things:

- **Understand:** collect commands, entrypoints, repo structure, boundaries, and validation paths.
- **Act:** generate `AGENTS.md`, reports, task cards, guard rules, before/after notes, and action plans.
- **Verify:** give agents and CI a clear checklist so changes are easier to review.

简单说，它帮大模型做三件事：

- **看懂仓库：** 整理命令、入口、结构、边界和验证路径。
- **按规则行动：** 生成 `AGENTS.md`、报告、任务卡、guard rules、前后对比和行动计划。
- **更容易验证：** 给 Agent 和 CI 一份明确清单，让改动更容易 review。
...
The default path is LLM-first and requires no model signup or API key. Local-only mode is available for privacy, outages, or other emergency fallback cases.

默认路径优先使用大模型，而且不需要用户注册模型平台或申请 API Key；纯本地模式只建议在隐私、网络故障或其他应急场景下使用。
...
| LLM recommendations | Primary analysis path. Uses the managed no-key endpoint by default, BYOK when configured, and local fallback only when needed. | 主要分析路径。默认使用免 key 托管端点，也支持自带 key；只有必要时才本地兜底。 |

## Quick Start / 快速开始

The default command is LLM-first. It sends bounded repo evidence to the maintainer-hosted managed LLM endpoint, so common users can run it without creating any model account or API key.

默认命令是 LLM-first：它会把有限仓库证据发给维护者托管的大模型端点，所以普通用户可以直接运行，不需要创建模型账号或申请 API Key。
...
The scanner reads repository shape, docs, scripts, and lightweight file evidence locally so the LLM has structured context instead of a raw dump. Recommendations are LLM-first: the CLI tries the maintainer-hosted managed endpoint by default, retries with summary-only context if needed, and uses deterministic local output only as an outage/privacy fallback. It still does not claim to review every line like a full human reviewer.

扫描器会在本地读取仓库结构、文档、脚本和轻量文件证据，让大模型拿到结构化上下文，而不是原始文件乱灌。建议生成是 LLM-first：CLI 默认请求维护者托管端点，必要时会用摘要模式重试，只有在服务不可用或用户出于隐私需要时才退回本地确定性输出。它仍然不会假装完整审查了每一行代码。

## Managed LLM Mode / 托管大模型模式

By default, `agent-ready-kit` uses a managed LLM path as the main analysis route:

默认情况下，`agent-ready-kit` 会把托管大模型作为主要分析路径：
...
jobs:
  score:
    ...
    steps:
      ...
      - uses: chen9965/agent-ready-kit@v0.6.3
        with:
          min-score: 70
```

The action runs LLM-first analysis by default, writes a bilingual Markdown report to the GitHub Actions step summary, and fails when the score is below `min-score`. Set `llm: false` only for local-only emergency mode.

Action 默认走 LLM-first 分析，会在 GitHub Actions Step Summary 里写入双语报告；分数低于 `min-score` 时失败。只有需要纯本地应急模式时才设置 `llm: false`。

## Commands / 命令

| Command | What it does | 中文说明 |
| --- | --- | --- |
| `agent-ready [path]` | Runs LLM-first repo analysis, then scores docs, tests, scripts, CI, repo map, safety, and onboarding. | 优先用大模型分析仓库，再为文档、测试、脚本、CI、仓库地图、安全和上手体验评分。 |
...
- **LLM-first by default:** the CLI and GitHub Action treat the model as the main analysis path, not an optional add-on.
...
- **默认 LLM-first：** CLI 和 GitHub Action 都把大模型作为主要分析路径，而不是可选点缀。
...
- `--out` 写出 `scan.json`、`report.md`、`before-after.md` 和 `action-plan.md`，方便分享或接入 CI。
