...
**Scan a repository, score how ready it is for AI coding agents, and generate the files agents need to work safely.**

**扫描仓库、给出 AI Agent 就绪度分数，并生成 Codex、Claude Code、Cursor 这类编码代理真正需要的工作材料。**

**Default path: no model signup and no API key required.**

**默认使用路径不需要注册模型平台，也不需要用户自己申请 API Key。**

`agent-ready-kit` is a local CLI and GitHub Action for making existing repositories easier for coding agents to use. It checks whether a repo clearly exposes setup commands, test commands, CI, project structure, edit boundaries, and `AGENTS.md`.

`agent-ready-kit` 是一个本地 CLI 和 GitHub Action，目标是让已有仓库更容易被 AI 编码代理接手。它会检查仓库有没有讲清楚：怎么安装、怎么测试、CI 在哪里、项目结构是什么、哪些边界不要乱碰、有没有 `AGENTS.md`。

In plain language, it does three things:

- **Scan:** find missing agent-facing signals in docs, scripts, tests, CI, repo map, and ignore rules.
- **Score:** produce an `Agent Ready Score` with clear reasons.
- **Generate:** write `AGENTS.md`, reports, task cards, guard rules, before/after notes, and action plans.

简单说，它做三件事：

- **扫描：** 找出文档、脚本、测试、CI、仓库地图、忽略规则里缺少的 Agent 协作信号。
- **评分：** 输出可解释的 `Agent Ready Score`。
- **生成：** 写出 `AGENTS.md`、报告、任务卡、guard rules、前后对比和行动计划。
