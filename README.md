...
After scanning, it gives you a score and can generate the missing agent-facing files. For large repositories, `scan --out` also writes a portable artifact bundle: raw JSON, Markdown report, prioritized action plan, and a before/after explanation. If a repository is mostly code with little documentation, `--llm-code` can send sampled code context to an OpenAI-compatible model so recommendations are based on the actual project shape, not only file names.

扫描后，它会给仓库一个就绪度分数，并且可以生成 Agent 直接能看的说明文件和任务清单。针对大仓库，`scan --out` 还会落地一组可分享产物：原始 JSON、Markdown 报告、优先级行动计划和使用前后对比说明。如果仓库几乎只有代码、缺少文档，`--llm-code` 可以把采样后的代码上下文发给 OpenAI 兼容模型，让建议基于真实项目结构，而不只是文件名。
...
| Optional LLM recommendations | Model-written suggestions based on scan summary, with optional sampled-code mode. | 可选大模型增强建议，默认只发扫描摘要，也可显式开启代码采样模式。 |
...
Use LLM-assisted scanning for code-only repositories:

给“只有代码、文档很少”的仓库加大模型辅助：

```bash
set AGENT_READY_LLM_API_KEY=your_key
set AGENT_READY_LLM_MODEL=provider/model-name
npx @chent6767/agent-ready-kit scan . --llm-code --out .agent-ready
```
...
`--llm` sends the score, detected signals, and findings summary. It does not send source files.

`--llm` 只发送评分、检测信号和发现摘要，不发送源码文件。

For code-only repositories, use `--llm-code`. It sends a bounded, sampled code context: file tree, entrypoint candidates, test candidates, and short excerpts from selected config/source files. It skips obvious secret-like paths and content, and it does not claim to review the full source tree.

如果仓库几乎只有代码，可以使用 `--llm-code`。它会发送有限的采样代码上下文：文件树、入口候选、测试候选，以及少量配置/源码片段。它会跳过明显像密钥的路径和内容，也不会宣称完整审查了全仓库源码。

```bash
set AGENT_READY_LLM_API_KEY=your_key
set AGENT_READY_LLM_MODEL=provider/model-name
npx @chent6767/agent-ready-kit scan . --llm-code --llm-max-files 12 --llm-max-chars 18000 --markdown
```
...
| `agent-ready scan [path] --out .agent-ready` | Writes `scan.json`, `report.md`, `before-after.md`, and `action-plan.md`. | 写入 `scan.json`、`report.md`、`before-after.md` 和 `action-plan.md`。 |
...
| `agent-ready scan [path] --llm-code --out .agent-ready` | Sends sampled code context to the configured model and writes artifacts. | 向已配置模型发送采样代码上下文，并写入扫描产物。 |
...
- **LLM-aware when needed:** code-only repositories can opt into sampled-code analysis.
...
- `scan --out` writes `scan.json`, `report.md`, `before-after.md`, and `action-plan.md`.
...
- `scan --out` 直接写出 `scan.json`、`report.md`、`before-after.md` 和 `action-plan.md`。
